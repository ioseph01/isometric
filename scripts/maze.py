...
class Maze:
    def __init__(self, game, width, height, gem_asset, color_table=None, stair_prob=0, room_attempts=0, sparsity=[0,0], elevator_prob=0,
        ...
        self.init_settings = {
            ...
            'glass_prob': glass_prob, 'enemies_cooldown':enemies_cooldown
            }
        ...
        for i in range(10):
            ...
            if self.connectivity() and self.trace([1,1], [self.w - 2, self.h - 2]):
                break
        else:
            raise RuntimeError("Maze generation failed to produce a connected maze")
    ...
    def enemy_cooldown(self, x=0, y=0):
        if self.enemy_tile_cooldown == 0:
            ...
            return int(round(15 + ((result + 1) / 2) * 385))
    ...
    def fix_maze(self, maze):
        ...
        def get_cell(x,y, dx, dy):
            if cell_valid([x + dx, y + dy],self):
                return self.maze[y + dy][x + dx]
            dx,dy = advance(dx,dy)
            if cell_valid([x + dx, y + dy], self):
                return self.maze[y + dy][x + dx]
            return None
            ...
            for cell_i, cell in enumerate(row):
                if cell is not None:
                    if cell.type == 'Elevator':
                        top = check(get_cell(cell.x, cell.y,-1,0), get_cell(cell.x,cell.y,0,-1),min)
                        bottom = check(get_cell(cell.x, cell.y,1,0), get_cell(cell.x,cell.y,0,1),max)
    ...
    def combine(self, layout, other, sparsity, friendlies_prob=0, enemies_prob=0, glass_prob=0, preserve_current=False):
        ...
        new_tiles = []
        weights = [1,glass_prob,friendlies_prob,enemies_prob]
            ...
            for x in range(1, len(layout[y]) - 1):
                ...
                if (layout[y][x] is None or None is other[y][x]):
                    if random.randint(0,100) < sparsity[0]:
                        tile = random.choices([makeTile,makeGlassTile,makePlayerTile,makeEnemyTile], weights=weights,k=1)[0]
            ...
            for x in range(self.w):
                ...
                if tile is not None:
                    ...
                    if tile.type in ("Enemy_Tile", "Portal_Tile"):
                        Tile(self,tile.z1,tile.x,tile.y).render(screen, rect, offset)
    ...
    def border_check(self, player, direction):
        ''' Returns next coords to move to and the render difference '''
        if direction == [0,0]:
        ...
        for i in range(n):
            ...
            if cell_valid(pos, self):
                ...
                if dx <= 10 and dy <= 10 and dx + dy + abs(new_tile - tile) < locs[0]:
                    if i == 0 and abs(new_tile - tile) > 2:
                        continue
